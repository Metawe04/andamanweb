...
class Image
{
    ...
    {
        if($filename && is_file(($filename = Yii::getAlias('@storage/web/source') . $filename)))
        {
            $info = pathinfo($filename);
            $thumbName = $info['filename'] . '-' . md5( filemtime($filename) . (int)$width . (int)$height . (int)$crop ) . '.' . $info['extension'];
            $thumbFile = Yii::getAlias('@storage/web') . DIRECTORY_SEPARATOR . Upload::$UPLOADS_DIR . DIRECTORY_SEPARATOR . 'thumbs' . DIRECTORY_SEPARATOR . $thumbName;
            $thumbWebFile = '/storage/' . Upload::$UPLOADS_DIR . '/thumbs/' . $thumbName;
        ...
        return '/storage/' . Upload::$UPLOADS_DIR . '/thumbs/' . $filename;
    }
